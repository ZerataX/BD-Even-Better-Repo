...
evenBetterRepo.prototype.load = function(){
    ...
    evenBetterRepo.installTheme = function(url){
        var themeName = url.substr(url.lastIndexOf('/') + 1);
        if (_os == "win32") {
            var dest = process.env.APPDATA + "\\BetterDiscord\\themes\\" + decodeURI(url.substr(url.lastIndexOf('/') + 1));
        }else if (_os == "linux"){
    ...
    evenBetterRepo.installPlugin = function(url){
        var pluginName = url.substr(url.lastIndexOf('/') + 1);
        if (_os == "win32") {
            var dest = process.env.APPDATA + "\\BetterDiscord\\plugins\\" + decodeURI(url.substr(url.lastIndexOf('/') + 1));
        }else if (_os == "linux") {
